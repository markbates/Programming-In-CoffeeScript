(function() {
  var build, exec, fs, print, spawn, watchit, _ref,
  ...
  task('test', 'Run the test suite (and re-run if anything changes)', function() {
    ...
    return build(function() {
      var runTests, watchTargets;
      (runTests = function() {
        var runNextTestSuite, suiteIndex, suiteNames;
        if (suite != null) suite.kill();
        suiteNames = ['DevelopmentIntegration', 'ProductionIntegration', 'AbsoluteIntegration', 'RemoteIntegration', 'BenchmarkDynamic', 'BenchmarkStatic'];
        suiteIndex = 0;
      ...
      watchTargets = function() {
        var callback, target, targets, _i, _j, _len, _results;
        targets = 2 <= arguments.length ? __slice.call(arguments, 0, _i = arguments.length - 1) : (_i = 0, []), callback = arguments[_i++];
        _results = [];
        for (_j = 0, _len = targets.length; _j < _len; _j++) {
          target = targets[_j];
          _results.push(watchit(target, {
            include: true
          }, function(event) {
            if (event !== 'success') return callback();
          }));
        }
        return _results;
      };
      watchTargets('src', function() {
        return build(runTests);
      });
      return watchTargets('test', 'Cakefile', runTests);
    });
  });
